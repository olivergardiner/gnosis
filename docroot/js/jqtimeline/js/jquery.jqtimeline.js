...
(function($) {
	var pluginName = 'jqTimeline',
		defaults = {
			...
			contextmenu : null //Handler for click event for event
		},
	...
	jqTimeLine.prototype._getEventMarkup = function(e){
		...
		var $retHtml = $('<div class="event' + eventClass + '" id="event_'+e.id+'" style="left:'+leftVal+'px">&nbsp;</div>').data('event',e);
		$retHtml.data('eventInfo',_this._aEvents[e.id]);
		if(_this.options.click){
			_this._addEventListner($retHtml,'click');
		}
		if(_this.options.contextmenu){
			_this._addEventListner($retHtml,'contextmenu');
		}
	...
	jqTimeLine.prototype._addEventListner = function($retHtml,sEvent){
		...
		if(sEvent == 'contextmenu'){
			alert("CONTEX MENU CLICKED");
		}
	}
